...
// compiler/index.ts
var compiler_exports2 = {};
__export(compiler_exports2, {
  PRESETS: () => PRESETS2,
  astro: () => astro,
  babel: () => babel,
  compile: () => compile2,
...
module.exports = __toCommonJS(compiler_exports2);
init_cjs_shims();
...
var decompress = () => {
  return async function decompress2(ctx, next3) {
...
var transform = async (instance, meta, options, code, id) => {
  ...
  try {
    ...
    instance.meta.stats.components += withBabelOptions.stats.components;
    instance.meta.stats.captures += withBabelOptions.stats.captures;
...
var unplugin = (0, import_unplugin.createUnplugin)((options, meta) => {
  ...
  return {
    ...
    async transform(code, id) {
      return transform(instance, meta, options || {}, code, id);
    },
    buildEnd() {
      var _a2;
      if (options == null ? void 0 : options.skipTransform)
...
var findDir = (dir, name) => {
  let currDir = (0, import_node_path4.join)(dir, name);
  if ((0, import_node_fs3.existsSync)(currDir))
    return currDir;
  currDir = (0, import_node_path4.join)(dir, "src", name);
  if ((0, import_node_fs3.existsSync)(currDir))
    return currDir;
  return null;
};
var next = (options = {}) => {
  return (nextConfig = {}) => {
    ...
    const injectLoader = (glob, as) => {
      ...
      (_a = nextConfig.experimental).turbo ?? (_a.turbo = {});
      (_b = nextConfig.experimental.turbo).rules ?? (_b.rules = {});
      const rules = nextConfig.experimental.turbo.rules;
      rules[glob] ?? (rules[glob] = {});
      const rule = rules[glob];
      rule.as = as;
      rule.loaders ?? (rule.loaders = []);
      rule.loaders.push({
    ...
    return {
      ...
      webpack: (config, webpackOptions) => {
        var _a;
        const { dir } = webpackOptions;
        const hasAppDir = ((_a = nextConfig.experimental) == null ? void 0 : _a.appDir) ?? findDir(dir, "app");
        config.plugins.push(
          unplugin.webpack({
            framework: "next",
            rsc: hasAppDir,
      ...
      // TEMPORARY DISABLED
      // async rewrites() {
      //   const prevRewrites = (await nextConfig.rewrites?.()) || [];
      //   if (options.production?.flags?.disableNextRewrites) return prevRewrites;
      //   const obj = {
      //     source: "/__anya",
      //     destination: `${options.production?.url ?? "https://lint.million.dev"}/api/v1/ingest/ingest`,
      //   };
      //   if (Array.isArray(prevRewrites)) {
      //     return [...prevRewrites, obj];
      //   } else if (typeof prevRewrites === "object" && prevRewrites) {
      //     return {
      //       ...prevRewrites,
...
// compiler/index.ts
var PRESETS2 = PRESETS;
var esbuild2 = esbuild_default;
var rollup2 = rollup_default;
